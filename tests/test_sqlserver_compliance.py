import unittest
from testcontainers.sqlserver import SqlServerContainer
from src.app import MsSqlServerConnector
...
class TestMsSqlServerConnector(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.sql_server_connector = MsSqlServerConnector()
        cls.conn = cls.sql_server_connector.connect(
            ...
            "master",
            cls.sql_server_container.get_username(),
            cls.sql_server_container.get_password()
        )
    ...
    def setUp(self):
        # Initialize the MsSqlServerConnector instance
        self.sql_server_connector = MsSqlServerConnector()

    def test_scan_for_sensitive_data(self):
        # Happy path test for scan_for_sensitive_data
        with self.conn.cursor() as cursor:
            sensitive_data = self.sql_server_connector.scan_for_sensitive_data(cursor)
        # Expecting 1 row of sensitive data
        self.assertEqual(len(sensitive_data), 1)

    def test_check_access_controls(self):
        # Happy path test for check_access_controls
        with self.conn.cursor() as cursor:
            access_controls = self.sql_server_connector.check_access_controls(cursor)
    ...
    def test_check_encryption(self):
        ...
        with self.conn.cursor() as cursor:
            encryption_status = self.sql_server_connector.check_encryption(cursor)
        # Expecting a boolean value
    ...
    def test_check_activity_monitoring(self):
        ...
        with self.conn.cursor() as cursor:
            activity_monitoring_status = self.sql_server_connector.check_activity_monitoring(cursor)
        # Expecting a boolean value
